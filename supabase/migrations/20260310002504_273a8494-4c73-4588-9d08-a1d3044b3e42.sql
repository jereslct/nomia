...
INSERT INTO storage.buckets (id, name, public) VALUES ('pay-stubs', 'pay-stubs', true) ON CONFLICT (id) DO NOTHING;
...
-- Storage policies for pay-stubs
DO $$ BEGIN
CREATE POLICY "Authenticated users can upload pay stubs"
  ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'pay-stubs');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
CREATE POLICY "Authenticated users can view pay stubs"
  ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id = 'pay-stubs');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
CREATE POLICY "Authenticated users can delete pay stubs"
  ON storage.objects FOR DELETE TO authenticated
  USING (bucket_id = 'pay-stubs');
EXCEPTION WHEN duplicate_object THEN null;
END $$;
